...
def process_qr_data(qr_data):
    ...
    try:
        ...
        conn.commit()
        update_status(f"【退館】{user_name}さん 退館しました", "success")
...
def generate_frames():
    ...
    update_status("退館用端末です。QRコードをかざしてください", "info")
        ...
        try:
            ...
            if bbox is not None:
                ...
                for j in range(n):
                    ...
                    cv2.line(frame, pt1, pt2, (0, 0, 255), 3) # バウンディングボックスを赤色に

                if data:
                    pt1 = tuple(map(int, bbox[0][0]))
                    cv2.putText(frame, data, (pt1[0], pt1[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
